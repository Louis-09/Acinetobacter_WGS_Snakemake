...
SPECIES_NAMES = {
    ...
    "acinetobacter_haemolyticus": "Acinetobacter haemolyticus",
    "acinetobacter_pittii": "Acinetobacter pittii",
    "klebsiella_pneumoniae": "Klebsiella pneumoniae",
    "klebsiella_variicola": "Klebsiella variicola",
    "klebsiella_aerogenes": "Klebsiella aerogenes",
    "klebsiella_quasipneumoniae": "Klebsiella quasipneumoniae",
...
for sample in SAMPLES:
    ...
    path = RESULTS / sample / "species_identification" / "best_hit.tsv"
    ...
    if path.exists():
        ...
        if line:
            fields = line.split("\t")
            SPECIES[sample] = fields[1]
        else:
            SPECIES[sample] = "Unknown"
...
for sample in SAMPLES:
    ...
    ws.append([
        sample,
        SPECIES_NAMES.get(SPECIES.get(sample, ""), SPECIES.get(sample, "Unknown")),
...
for sample in SAMPLES:
    ...
    path = RESULTS / sample / "vfdb" / f"{sample}_vfdb_filtered.tsv"
...
if iqtree_dir.exists():
    ...
    for species_dir in sorted(iqtree_dir.iterdir()):
        ...
        n_samples = len(samples_in_species)
        ...
        if iqtree_report.exists():
            ...
            # UFBoot
            if re.search(r"ultrafast bootstrap", text, re.IGNORECASE):
                ws_tree.append([
                    species_name,
                    n_samples,
                    "Ultrafast bootstrap",
            ...
            # SH-aLRT
            if re.search(r"SH-aLRT", text, re.IGNORECASE):
                ws_tree.append([
                    species_name,
                    n_samples,
                    "SH-aLRT",
            ...
            # Soporte de ramas
            if re.search(
                r"Numbers in parentheses are SH-aLRT support.*ultrafast bootstrap support",
                text,
                re.IGNORECASE
            ):
                ws_tree.append([
        ...
        # Si no hay suficientes secuencias
        if n_samples < 4:
            ws_tree.append([
                species_name,
                n_samples,
...
ws_tree.column_dimensions["A"].width = 30
ws_tree.column_dimensions["B"].width = 18
ws_tree.column_dimensions["C"].width = 38
ws_tree.column_dimensions["D"].width = 100
